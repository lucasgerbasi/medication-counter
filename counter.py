...
def contador_medicamentos_final(caminho_imagem):
    ...
    # 2. Correção de Iluminação (CLAHE)
    ...
    # 4. Morfologia e Separação de Agrupamentos
    ...
    for c in cnts:
        ...
        if area > 200:
            ...
            # Expansão para compensar a erosão
            ...
            # UI: Borda elegante (USANDO cv2.LINE_AA para suavizar o serrilhado)
            cv2.drawContours(output, [box], 0, (255, 200, 50), 2, cv2.LINE_AA)

            # Ponto central estilo "alvo"
            cX, cY = int(center[0]), int(center[1])
            cv2.circle(output, (cX, cY), 3, (255, 255, 255), -1, cv2.LINE_AA)
            cv2.circle(output, (cX, cY), 4, (255, 200, 50), 1, cv2.LINE_AA)

            # UI: Texto e Badge Minimalista
            texto = f"{contador}"
            font = cv2.FONT_HERSHEY_DUPLEX  # Fonte mais simétrica e profissional
            escala = 0.6
            espessura = 1
